...
namespace Repos.Lib
{
    ...
    public class SqlHelper
    {
        ...
        /// <returns></returns>
        public static bool ValidateConnectionStr(string connectionStr)
        {
            try
            {
                var result = ExecuteScalar(connectionStr, "select getdate()");
                return result != null;
            }
            catch { }
            return false;
            ...
            CommandType commandType, string commandText, params SqlParameter[] parms)
        {
            ...
            if (commandText.IndexOf("@", StringComparison.Ordinal) > 0)
            {
                ...
                var index = commandText.IndexOf("where ", StringComparison.Ordinal);
                if (index < 0)
                {
